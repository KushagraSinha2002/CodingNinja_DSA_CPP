...
Output Format:
Information of all the nodes in the Binary Tree will be printed on a different line where
each node will follow a format of D:L:X,R:Y, without any spaces in between.
...
void printLevelWise(BinaryTreeNode<int> *root) {
    ...
    queue<BinaryTreeNode<int>*> output;
    output.push(root);

    while(output.size() != 0){
        BinaryTreeNode<int>* front = output.front();
        output.pop();
        cout<<front->data<<":";
        if(front->left != NULL){
